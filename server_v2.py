...
_auto_executor_running = False
_auto_executor_last_check = ''
_auto_executor_last_result = None
_suggestions_generated_today = False

# paper_trading module is lazy-loaded inside API functions to avoid circular import
...
def api_paper_execute():
    """执行价格匹配：盘中自动/手动触发执行今日符合条件的建议。"""
    try:
        from paper_trading import check_and_execute as _check_exec
        result = _check_exec()
        ...
        return api_response(
            ...
            message=(
                f"执行完成，今日执行 {result.get('executed_count', 0)} 笔"
                + (f"，{result.get('skipped_count', 0)} 条因价格未达条件跳过"
                   if result.get('skipped_count', 0) > 0 else '')
            ),
        )
    except Exception as e:
        import traceback as _tb
        return api_response(success=False, error=f"交易执行失败: {e}\n{_tb.format_exc()}", status_code=500)


@app.post("/api/v2/paper/generate")
def api_paper_generate():
    """手动触发：生成今日预测和交易建议（仅生成，不执行）。"""
    try:
        from paper_trading import generate_today_suggestions as _gen
        result = _gen()
        if result.get('success'):
            global _suggestions_generated_today
            _suggestions_generated_today = True
            return api_response(
                data=result,
                message=f"建议生成完成，共 {result.get('generated_count', 0)} / {result.get('codes_count', 0)} 只股票"
            )
        else:
            return api_response(
                success=False,
        ...
        import traceback as _tb
        return api_response(success=False, error=f"建议生成失败: {e}\n{_tb.format_exc()}", status_code=500)


@app.get("/api/v2/paper/auto-status")
def api_paper_auto_status():
    """查询自动执行状态。"""
    from paper_trading import TODAY as _today
    db = get_db()
    pending = db.execute(
    ...
    executed = db.execute(
        "SELECT COUNT(*) FROM paper_suggestions WHERE date=? AND executed=1",
        [_today]
    ).fetchone()[0]
    # 直接从数据库判断今日是否已有建议，不依赖内存变量（重启后仍准确）
    has_suggestions = db.execute(
        "SELECT COUNT(*) FROM paper_suggestions WHERE date=?",
        [_today]
    ).fetchone()[0] > 0
    db.close()
    return api_response(data={
        'running': _auto_executor_running,
        'last_check': _auto_executor_last_check,
        'suggestions_generated': has_suggestions or _suggestions_generated_today,
        'pending_count': pending,
        'executed_count': executed,
        'last_result': _auto_executor_last_result,
    })

@app.get("/api/v2/paper/suggestions-history")
def api_paper_suggestions_history(date: str = Query(default=None), code: str = Query(default=None), days: int = Query(default=30)):
    """查询历史建议快照（每日生成时的初始建议）。"""
    from db_helper import get_paper_suggestions_history
    data = get_paper_suggestions_history(date=date or None, code=code or None, days=days)
    return api_response(data=data, count=len(data))
...
@app.get("/api/v2/paper/verify")

def api_paper_verify():

    """验证纸面交易数据的一致性与完整性。"""

    try:

        from paper_trading import verify_paper_consistency as _verify

        result = _verify()

        return api_response(data=result, consistent=result.get("consistent", False))

    except Exception as e:

        import traceback as _tb

        return api_response(success=False, error=f"校验失败: {e}\n{_tb.format_exc()}", status_code=500)


@app.post("/api/v2/paper/intraday/collect")
def api_intraday_collect():
    """触发分时数据采集（调用 collect_intraday.py once），然后返回最新数据。"""
    try:
        import subprocess as _sp
        _sp.run([sys.executable, os.path.join(ROOT, "scripts", "collect_intraday.py"), "once"],
                cwd=ROOT, capture_output=True, timeout=120)
        return api_response(data={"message": "分时数据采集完成"})
    except Exception as e:
        import traceback as _tb
        return api_response(success=False, error=f"采集失败: {e}\n{_tb.format_exc()}", status_code=500)
...
def api_pattern_rules_init():
    ...
    try:
        ...
        python = _sys.executable or get_python()
...
def auto_executor_loop():
    """后台自动执行线程。每60秒检查一次：
    - 开盘前(9:00-9:30)：生成今日建议
    - 盘中(9:30-11:30, 13:00-15:00)：价格匹配执行
    - 收盘后(15:00-15:05)：最后一次执行，重置明日标记
    - 其他时间：休眠
    """
    global _auto_executor_running, _auto_executor_last_check, _auto_executor_last_result, _suggestions_generated_today
    _auto_executor_running = True
    _suggestions_generated_today = False

    def _is_weekend(dt):
        return dt.weekday() >= 5

    def _is_before_market(dt):
        return dt.hour < 9 or (dt.hour == 9 and dt.minute < 30)

    def _is_market_hours(dt):
        return (dt.hour == 9 and dt.minute >= 30) or (dt.hour == 10) or (dt.hour == 11 and dt.minute <= 30) \
            or (dt.hour == 13 and dt.minute >= 0) or (dt.hour == 14)

    def _is_closing_period(dt):
        return dt.hour == 15 and dt.minute <= 5
    ...
    while True:
        try:
            ...
            with _auto_executor_lock:
                if _is_before_market(now) and not _suggestions_generated_today:
                    # 开盘前：生成今日建议
                    from paper_trading import generate_today_suggestions as _gen
                    _gen_result = _gen()
                    if _gen_result.get('success'):
                        _suggestions_generated_today = True
                        print(f"[AutoExec] 今日建议已自动生成 ({_gen_result.get('generated_count', 0)} 只)")
                    _auto_executor_last_check = now.strftime('%H:%M:%S')
                    _auto_executor_last_result = _gen_result

                elif _is_market_hours(now) or _is_closing_period(now):
                    # 盘中或收盘前后：执行价格匹配
                    from paper_trading import check_and_execute as _check_exec
                    _exec_result = _check_exec()
                    _auto_executor_last_check = now.strftime('%H:%M:%S')
                    _auto_executor_last_result = _exec_result

                    if _exec_result.get('executed_count', 0) > 0:
                        print(f"[AutoExec] {now.strftime('%H:%M:%S')} 执行 {_exec_result['executed_count']} 笔"
                              + (f"，跳过 {_exec_result.get('skipped_count', 0)} 笔" if _exec_result.get('skipped_count', 0) > 0 else ''))

                    # 收盘后重置明日标记
                    if _is_closing_period(now):
                        _suggestions_generated_today = False

        except Exception as e:
            if _auto_executor_last_check:
                print(f"[AutoExec] 检查异常 ({now.strftime('%H:%M:%S')}): {e}")
            time.sleep(5)
...
if __name__ == "__main__":
    ...
    # 启动后台自动执行线程
    _executor_thread = threading.Thread(target=auto_executor_loop, daemon=True)
    _executor_thread.start()
    print(f"[AutoExec] 纸面自动交易线程已启动 (每60秒轮询)")
