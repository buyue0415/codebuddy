...
"""
import json, math, subprocess, re, os, sys
...
from db_helper import (upsert_kline_daily, upsert_kline_monthly, upsert_quotes,
...
for stock in watchlist:
    ...
    if kdata:
        ...
        d['kline_daily'][code] = bars
        try: upsert_kline_daily(code, bars)
        except Exception as e: print(f"  DB write kline failed: {e}")
...
try: clear_today_predictions(TODAY)
except Exception as e: print(f"  DB clear predictions failed: {e}")
...
for stock in watchlist:
    ...
    try: insert_daily_prediction(code, pred['date'], pred['prev_close'], pred['next_day'], pred['hourly'], pred['signals'])
    except Exception as e: print(f"  DB write pred failed: {e}")
    try: upsert_learning_params(code, d['learning_params'][code])
    except Exception as e: print(f"  DB write learning failed: {e}")
    try: upsert_accuracy_stats(code, 'last_20', d['accuracy_stats'].get(code, {}).get('last_20', empty_acc['last_20']))
    except Exception as e: print(f"  DB write accuracy failed: {e}")
    try: upsert_accuracy_stats(code, 'last_60', d['accuracy_stats'].get(code, {}).get('last_60', empty_acc['last_60']))
    except Exception as e: print(f"  DB write accuracy failed: {e}")
...
for stock in watchlist:
    ...
    if code not in d['seasonal']:
        ...
        try: upsert_seasonal(code, DEFAULT_SEASONAL)
    ...
    if code not in d['quotes'] and code in d.get('kline_daily', {}):
        ...
        if kd:
            ...
            try: upsert_quotes({code: d['quotes'][code]})
            except Exception as e: pass
