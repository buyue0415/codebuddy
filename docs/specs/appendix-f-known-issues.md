...
### 1. Python版本不一致
`server.py` 使用 Python 3.14.3，`scheduler.py` 使用 Python 3.13.12。不同版本可能导致行为差异。
- **影响**: 全局运行时稳定性
- **建议**: 统一到单一 Python 版本，从环境变量或 PATH 检测
...
### 6. 全局同步锁（单用户瓶颈）
`_refresh_in_progress` 锁是全局的，所有并发同步请求共享同一把锁。虽为单用户设计，但阻塞所有同步任务类型。
- **影响**: 并行新闻刷新和K线拉取无法同时执行
- **建议**: 按任务类型分层锁（news_lock / kline_lock / predict_lock）
...
### 10. 子进程超时时间分散硬编码
`server.py` 中各脚本超时（30s/60s/180s）分散在各函数中，不便于统一管理。
- **影响**: 未来调整超时需要搜索多处
- **建议**: 提取到 `config.json` 的 `timeouts` 配置节
