    ...
    def do_POST(self):
        ...
        try:
            ...
            if path == "/api/watchlist/add":
                ...
                # Initialize data for new stock
                ok1, out1 = run_script("upgrade_learning.py", 90)
                ok2, out2 = run_script("reinject_data.py", 10)

                json_response(self, {
                    "success": True,
                    "message": f"已添加 {name}({code})，数据初始化完成",
                    "watchlist": wl,
                ...
                ok, out = run_script("reinject_data.py", 10)
                json_response(self, {
                    "success": True,
                    "message": f"已移除 {code}",
                    "watchlist": wl,
            ...
            # === TRIGGERS ===
                ...
                ok, out = run_script("upgrade_learning.py", 90)
                ok2, out2 = run_script("reinject_data.py", 10)
                json_response(self, {
                    "success": ok and ok2,
                    "output": out[-1000:] if out else "",
                    "message": "预测生成完成，刷新页面查看" if ok else "预测生成失败",
                })
