"""
Refactored: iterate over watchlist for learning params upgrade and prediction generation.
"""
...
watchlist = d.get('watchlist', [{'code': '601166', 'name': 'default', 'market': 'sh'}])
...
def calc_signals(kdata):
    closes = [k[2] for k in kdata]; highs = [k[3] for k in kdata]; lows = [k[4] for k in kdata]; close = closes[0]
    ...
    return {'close':close,'atr':round(atr,3),
            'signals':{'macd':{'value':f'{macd_pct:+.2f}%','direction':macd_dir,'raw':round(macd_pct,2)},
...
def gen_pred(code,info,lp):
    ...
    w=lp['signal_weights'];bias=lp['hourly_bias'];sa=lp['seasonal_adj'];cb=lp['confidence_beta']
    ...
    bi=cb.get(dd,{'alpha':1,'beta':1})
    conf=max(0.4,round(bi['alpha']/(bi['alpha']+bi['beta']),2))
    ...
    for block,pct,note in hws:
        sd=dr*pct;hs=sum(w[s][block]*(1 if sig[s]['direction']=='bullish' else -1 if sig[s]['direction']=='bearish' else 0) for s in SIGNALS)
    ...
    return {'date':datetime.now().strftime('%Y-%m-%d'),'code':code,'prev_close':close,
            ...
            'hourly':hp,'signals':sig,'actual':{'open':None,'high':None,'low':None,'close':None,'next_day_direction_hit':None,'daily_range_hit':None,'hourly_hits':[None]*4}}
...
for stock in watchlist:
    code, name, mkt = stock['code'], stock['name'], stock['market']
    print(f'\n--- {name}({code}) ---')
    ...
    print(f'  pred: {pred["next_day"]["direction"]} conf={pred["next_day"]["confidence"]:.0%}')
...
print(f'\nDone. {len(watchlist)} stocks, {len(d["daily_predictions"])} predictions.')
